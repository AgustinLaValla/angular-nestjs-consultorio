...
export class StatisticsComponent implements OnDestroy {
  ...
  getChartsData(turnos: Turno[]) {
    ...
    barchartLabels.forEach(obra_social => {
      const mutualLength = turnos.filter(turno => turno.obraSocial._id === obra_social._id).length;
    ...
    donughnutLabels.forEach(consulta => {
      const consultaLength = turnos.filter(turno => turno.consulta._id === consulta._id).length;
      donughnutCharData.push(consultaLength);
    });


    this.barChart = {
      labels: barchartLabels.map(label => label.nombre),
    ...
    this.doughnutChart =
    {
      labels: donughnutLabels.map(label => label.nombre),
  ...
  getObrasSociales(turnos: Turno[]) {
    ...
    turnos.map(turno => {
      ...
      if (!mutualExists) {
        obrasSociales.push(turno.obraSocial);
      };
    });
  ...
  getMiembroServices(turnos: Turno[]): Servicio[] {
    let consultas: Servicio[] = [];
    turnos.map(turno => {
      const servicioExists = consultas.find(servicio => servicio._id === turno.consulta._id);
      if (!servicioExists) {
  ...
  generateColumnsData(pdf: PdfMakeWrapper, includeDiagnoses: true | undefined) {
    ...
    obrasSociales.map((mutual, idx) => {
      ...
      this.turnos.map(turno => {
        if (turno.obraSocial._id === mutual._id && includeDiagnoses) {
          columns.push(
            [
              turno.apellido,
              turno.nombre,
              ...
              turno.numeroDeAfiliado ? turno.numeroDeAfiliado : '',
        ...
        } else if (turno.obraSocial._id === mutual._id && includeDiagnoses === undefined) {
          columns.push(
            [
              turno.apellido,
              turno.nombre,
      ...
      pdf.add(new Txt(`${mutual.nombre}`).alignment('center').bold().fontSize(14).end);
