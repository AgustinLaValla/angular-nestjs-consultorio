...
import { loadCloseTable, loadGetPacientTurnos, loadGetSinglePacient, loadResetPacientStoreData, loadResetPacientTurnoStore, hidePacientData, subscribeLoading, loadGetPacientTurnosSuccess } from 'src/app/store/actions';
...
export class PacientDataComponent implements OnInit, OnDestroy {
  ...
  getPacient() { 
    this.pacientSubs$ = this.store.select( getPacient ).pipe(
      ...
      tap((pacient) => this.store.dispatch(loadGetPacientTurnosSuccess({pacient_turnos:pacient.turnos})))
    ).subscribe();
  };
