...
export class EspecialistasListComponent implements  OnDestroy {
  ...
  showTurnos(especialista: Miembro) {
    ...
    if (this.headerDescription === 'Especialistas') {
      this.store.dispatch(ShowTurnosData({ especialistaId: especialista._id }));
      ...
      setTimeout(() => {
        this.store.dispatch(loadOpenTable({ especialistaId: especialista._id, tableType: 'Turnos Pasados', counter: 15 }));
      }, 50);
    } else if(this.headerDescription === 'Estadisticas') { 
      this.store.dispatch(loadShowStatistics({especialistaId:especialista._id}));
    };
    this.store.dispatch(loadGetEspecialidadByItsName({ nombreEspeciliadad: especialista.especialidad.nombreEspecialidad }));
  };
